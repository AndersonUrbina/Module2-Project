...
class Program
{
    ...
    static void Main()
    {
        ...
        var key = new List<string>(operationCount.Keys);
        for (int i = 0; i < key.Count; i++)
        {
            Console.WriteLine($"{key[i]}: {operationCount[key[i]]}");
        }
    ...
    {
        double a = ReadDouble("Enter first number: ");
        double b; //Variable was created but not assigned
        do
        {
            b = ReadDouble("Enter second number (non-zero): ");
            if (b == 0)
                Console.WriteLine("Cannot devide by zero");
        } while (b == 0); //Added loop to ensure b is not zero
